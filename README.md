...
- 本项目仅用于管理使用者本人账号下的收藏夹，不应用于第三方账号、批量采集、商业数据抓取、刷量、绕过平台限制或影响平台服务稳定性。
- 本项目不是 Bilibili 官方 API、官方客户端或官方合作项目，也未获得 Bilibili 背书、赞助或授权。
- 脚本不要求用户填写 Cookie、SESSDATA、csrf 或账号密码；浏览器请求只在已登录的 Bilibili 页面上下文内发起。
- 请遵守 Bilibili 用户协议、社区规则和所在地法律法规。平台可能对自动化操作触发风控，使用前请理解并接受相关风险。
